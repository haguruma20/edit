...
{
    return(lhs.zcoord<rhs.zcoord || (lhs.zcoord==rhs.zcoord && lhs.ycoord<rhs.ycoord) || (lhs.zcoord==rhs.zcoord && lhs.ycoord==rhs.ycoord && lhs.xcoord<rhs.xcoord));
};

point3 operator + (const point3& lhs,const point3& rhs)
{
  point3 pt(lhs.xcoord+rhs.xcoord,lhs.ycoord+rhs.ycoord,lhs.zcoord+rhs.zcoord);
  return pt;
};
...
{
  return lhs.xcoord*rhs.xcoord+lhs.ycoord*rhs.ycoord+lhs.zcoord*rhs.zcoord;
};

//exterior product
point3 operator ^ (const point3& lhs,const point3& rhs)
{
  point3 pt(lhs.ycoord*rhs.zcoord-lhs.zcoord*rhs.ycoord,lhs.zcoord*rhs.xcoord-lhs.xcoord*rhs.zcoord,lhs.xcoord*rhs.ycoord-lhs.ycoord*rhs.xcoord);
  return pt;
};
...
{
  return pow((p1.xcoord-p2.xcoord)*(p1.xcoord-p2.xcoord)+(p1.ycoord-p2.ycoord)*(p1.ycoord-p2.ycoord)+(p1.zcoord-p2.zcoord)*(p1.zcoord-p2.zcoord),0.5);
};
...
{
  os<<sp.center.xcoord<<"\t"<<sp.center.ycoord<<"\t"<<sp.center.zcoord<<"\t"<<sp.radius<<"\n";
  return os;
}

bool operator < (const Sphere& lhs,const Sphere& rhs)
{
    return(lhs.center.zcoord<rhs.center.zcoord || (lhs.center.zcoord==rhs.center.zcoord && lhs.center.ycoord<rhs.center.ycoord) || (lhs.center.zcoord==rhs.center.zcoord && lhs.center.ycoord==rhs.center.ycoord && lhs.center.xcoord<rhs.center.xcoord));
};
...
Sphere intersection(const Sphere& sp1,const Sphere& sp2,const Sphere& sp3)
{
  ...
  point3 circle2pt=sp3.center-(normalvec*(sp3.center-circle1.center))*normalvec;
  double circle2rad=sqrt(sp3.radius*sp3.radius-distance(sp3.center,circle2pt)*distance(sp3.center,circle2pt));
...
//main function
Sphere another(Sphere& sp1, Sphere& sp2, Sphere& sp3, double new_radius, bool which)
{
  ...
  //intersection circle of sp1&sp2&sp3 growed as the size of "new_radius".
  ...
  //height is the distance between the plane and 4th pt (or 5th pt), which is the radius of midsp.
...
//return the new sphere which contacts sp1&sp2 and whose center is on x=x0 plane.
Sphere anotherx(Sphere& sp1,Sphere& sp2,double x0,double new_radius,bool which)
{
  //growed circle with the length of new_radius
  point3 gcircle1point(x0,sp1.center.ycoord,sp1.center.zcoord);
  double gcircle1radius=sqrt((sp1.radius+new_radius)*(sp1.radius+new_radius)-distance(gcircle1point,sp1.center)*distance(gcircle1point,sp1.center));
  Sphere gcircle1(gcircle1point,gcircle1radius);

  point3 gcircle2point(x0,sp2.center.ycoord,sp2.center.zcoord);
  double gcircle2radius=sqrt((sp2.radius+new_radius)*(sp2.radius+new_radius)-distance(gcircle2point,sp2.center)*distance(gcircle2point,sp2.center));
  Sphere gcircle2(gcircle2point,gcircle2radius);

  if(distance(gcircle1point,gcircle2point)==gcircle1radius+gcircle2radius)
    {
...
Sphere anothery(Sphere& sp1,Sphere& sp2,double y0,double new_radius,bool which)
{
  //growed circle with the length of new_radius
...
Sphere anotherz(Sphere& sp1,Sphere& sp2,double z0,double new_radius,bool which)
{
  //growed circle with the length of new_radius
